...
flowcell_ids = {
    ...
    "D[A-Z,0-9]{4}ACXX$" : (["HiSeq 1000", "HiSeq 1500", "HiSeq 2000", "HiSeq 2500"], "High Output (8-lane) v3 flow cell"),
    "H[A-Z,0-9]{4}ADXX$" : (["HiSeq 1500", "HiSeq 2000", "HiSeq 2500"], "Rapid Run (2-lane) v1 flow cell"),
    ...
    "D[A-Z,0-9]{4}$" : (["HiSeq 2000", "HiSeq 2500"], "Unknown HiSeq flow cell examined in SJ data"),
    "G[A-Z,0-9]{4}$" : (["MiSeq"], "MiSeq micro flow cell"),
    "H[A-Z,0-9]{5}RXX$" : (["NovaSeq"], "S1 flow cell"),
    "H[A-Z,0-9]{5}RXX$" : (["NovaSeq"], "SP flow cell"),
    "H[A-Z,0-9]{5}MXX$" : (["NovaSeq"], "S2 flow cell"),
    "H[A-Z,0-9]{5}SXX$" : (["NovaSeq"], "S4 flow cell")
}

upgrade_sets = [
  (set(["HiSeq 2000", "HiSeq 2500"]), ["HiSeq 2000", "HiSeq 2500"])
]
...
def derive_instrument_from_fcid(fcid):
  ...
  for pattern in flowcell_ids.keys():
    if re.search(pattern, fcid):
      matching_instruments |= set(flowcell_ids[pattern][0])
...
def resolve_instrument(possible_instruments_by_iid, possible_instruments_by_fcid):
  if len(possible_instruments_by_iid) == 0 and len(possible_instruments_by_fcid) == 0:
    return "unknown", "no confidence", "no match"
  ...
  return "conflicting information" if len(overlapping_instruments) == 0 else overlapping_instruments, "high confidence", "instrument id and flowcell id"


def main(ngsfile, n_samples=10000):
  ...
  instruments, confidence, based_on = resolve_instrument(possible_instruments_by_iid, possible_instruments_by_fcid)
