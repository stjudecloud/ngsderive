...
def resolve_instrument(possible_instruments_by_iid, possible_instruments_by_fcid, at_least_one_instrument_detected):
  if len(possible_instruments_by_iid) == 0 and len(possible_instruments_by_fcid) == 0:
    if at_least_one_instrument_detected:
      return set(["multiple instruments"]), "unknown confidence", "multiple instruments were detected in this sample"
    else:
  ...
  if len(overlapping_instruments) == 0:
    return set(["conflicting evidence"]), "high confidence", "Case needs triaging: {} by iid, {} by fcid".format(" or ".join(possible_instruments_by_iid), " or ".join(possible_instruments_by_fcid))
  else: 
